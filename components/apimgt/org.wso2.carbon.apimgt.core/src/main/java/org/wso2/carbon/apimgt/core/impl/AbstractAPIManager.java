...
public abstract class AbstractAPIManager implements APIManager {
    ...
    public AbstractAPIManager(String username, IdentityProvider idp, KeyManager keyManager, ApiDAO apiDAO,
                              ...
                              GatewaySourceGenerator gatewaySourceGenerator, APIGateway apiGatewayPublisher, StreamDAO streamDAO) {
    ...
    public AbstractAPIManager(String username, IdentityProvider idp, KeyManager keyManager, ApiDAO apiDAO,
        ...
        this(username, idp, keyManager, apiDAO, applicationDAO, apiSubscriptionDAO, policyDAO, apiLifecycleManager,
                labelDAO, workflowDAO, tagDAO, null, new GatewaySourceGeneratorImpl(), new APIGatewayPublisherImpl(), null);
    }
     ...
     * Checks whether the given API name is already registered in the system
     *
     * @param streamName A String representing an API name
     * @return true if the api name already exists and false otherwise
     * @throws APIManagementException if failed to check the context availability
     */
    @Override public boolean isStreamNameExist(String streamName) throws APIManagementException {
        try {
            return getStreamDAO().isStreamNameExists(streamName, username);
